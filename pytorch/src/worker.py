        ...
        try:
            ...
            if rank == 0:
                ...
                if seed is not None and input_seed is not None and result_file_str is not None:
                    ...
                    # dump the result
                    with open(os.path.join(result_file_dir, f"rank_{rank}.p"), "wb") as f:
                        pickle.dump(result_list, f)
                    # dump the intermediate layer output result
                    with open(os.path.join(result_file_dir, f"rank_{rank}_intermediate.p"), "wb") as f:
                        pickle.dump([all_dense_r, all_sparse_r_dict, all_sparse_weighted_r_dict, all_over_r], f)
